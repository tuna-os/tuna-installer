...
if "tuna_installer.windows.dialog_output" not in sys.modules:
    sys.modules["tuna_installer.windows.dialog_output"] = MagicMock()

from tuna_installer.views.done import apply_icon, do_reboot  # noqa: E402
...
class TestDoReboot(unittest.TestCase):
    ...
    def test_reboot_via_dbus_success(self):
        conn = MagicMock()
        with patch("tuna_installer.views.done.Gio.bus_get_sync", return_value=conn):
    ...
    def test_reboot_falls_back_to_subprocess_when_dbus_fails(self):
        with patch("tuna_installer.views.done.Gio.bus_get_sync", side_effect=Exception("no bus")), \
...
class TestApplyIcon(unittest.TestCase):
    ...
    def test_resource_uri_loads_texture(self):
        ...
        with patch("tuna_installer.views.done.Gdk.Texture.new_from_resource", return_value=mock_texture) as mock_load:
            apply_icon(status_page, "resource:///org/tunaos/Installer/images/tunaos.svg")
        mock_load.assert_called_once_with("/org/tunaos/Installer/images/tunaos.svg")
    ...
    def test_apply_icon_silently_ignores_errors(self):
        status_page = MagicMock()
        with patch("tuna_installer.views.done.Gdk.Texture.new_from_resource", side_effect=Exception("bad resource")):
            apply_icon(status_page, "resource:///org/tunaos/Installer/images/missing.svg")
...
class TestMainWindowIconExtraction(unittest.TestCase):
    ...
    def test_selected_icon_extracted_from_finals(self):
        finals = [
            {"hostname": "tunaos"},
            {"pretty_name": "Yellowfin", "icon": "resource:///org/tunaos/Installer/images/yellowfin.svg"},
        ]
        pretty_name = None
        selected_icon = None
        for f in finals:
        ...
        self.assertEqual(selected_icon, "resource:///org/tunaos/Installer/images/yellowfin.svg")
